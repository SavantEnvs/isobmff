...
#   build/dump                 sanitized + DWARF      -> target `dump`     (file box-parser fuzzer, cmd-mode)
#   build/fuzz_dump            sanitized + libFuzzer  -> target `dumpfuzz` (in-process SkipMDATData +
#                                                        box-tree stringify fuzzer)
...
# `dumpfuzz` (mayhem/fuzz_dump.cpp) covers the same SkipMDATData + box-tree-stringify behaviour as the
# `dump` CLI, in-process instead of black-box cmd-mode — `dump` itself is kept as-is (target parity +
# an end-to-end CLI sanity check), it's just no longer the only/primary signal for that code path.
...
# shellcheck disable=SC2086
$CXX $STD $SANITIZER_FLAGS $DEBUG_FLAGS $LIB_FUZZING_ENGINE -w $INC mayhem/fuzz_dump.cpp "${LIB[@]}" -o build/fuzz_dump & pids+=($!)
# shellcheck disable=SC2086
...
echo "build.sh: built build/fuzz_parser, build/dump, build/fuzz_dump, build/fuzz_Pad and oracle binaries"
